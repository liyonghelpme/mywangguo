StreetLight = class(FuncBuild)
function StreetLight:ctor(b)
    --self.inEffect = false
end

function StreetLight:initView()
    ...
    for i =0, 4, 1 do
        ...
        for j = 0, 4, 1 do
            ...
            if mapDict[key] ~= nil then
                ...
                --2 == 0.1  
                --0.3 6
                if ob.data ~= nil and ob.data.isProduct == 1 then
                    if dist == 2 then
...
function StreetLight:clearEffect()
    local map = getBuildMap(self.baseBuild) 
    local initX = 0
    local initY = -4
    local offX = 1
...
function StreetLight:finishBuild()
    removeSelf(self.baseBuild.gridNode)
    self.baseBuild.gridNode = nil
    --建造开始就已经确定效果了
    self:doEffect()
end
function StreetLight:removeSelf()
...
--当前位置无冲突 清理附近状态
function StreetLight:beginMove()
    --[[
...
function StreetLight:finishMove()
    --[[
    if self:checkBuildable() then
        --self:doEffect()
    end
