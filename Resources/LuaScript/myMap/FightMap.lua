...
function FightMap:checkWin()
    print("FightMap checkWin", Logic.challengeCity)
    ...
    if type(Logic.challengeCity) == 'table' and Logic.challengeCity.kind == 0 then
        if Logic.winArena then
            ...
            if reward[1] == 'equip' then
                ...
                addDialog(NewGoods.new(0, reward[2]))
            elseif reward[1] == 'goods' then
                --兵法书
                if reward[2] == 39 then
                    Logic.fightNum = Logic.fightNum+1
                    --addBanner("合战人数增加1")
                    addDialog(NewGoods.new(3, 1))
                else
                end
            ...
            addDialog(NewGoods.new(4, silver))
        end
    --挑战普通城堡胜利
    elseif Logic.ownCity[Logic.challengeCity] ~= nil or Logic.ownVillage[Logic.challengeCity] ~= nil then
        --addBanner("获取 胜利奖励！")
        --global.director:pushView(, 1, 0)
                ...
                for k, v in ipairs(cg.goods) do
                    ...
                    if not findTech then
                        if v == 46 then
                            ...
                            --addBanner("获得新兵种 弓兵")
                            addDialog(NewGoods.new(2, 1))
                        elseif v == 47 then
                            Logic.soldiers[3] = {1, 20} 
                            --addBanner("获得新兵种 魔法兵")
                            addDialog(NewGoods.new(2, 2))
                        elseif v == 48 then
                            Logic.soldiers[4] = {1, 10} 
                            --addBanner("获得新兵种 骑兵")
                            addDialog(NewGoods.new(2, 3))
                        elseif v == 38 then
