...
function BuildWorkMenu:updateView()
    ...
    elseif bData["funcs"] == FARM_BUILD then
        self.cw = colorWordsNode(getStr("normalFarm", {"[NAME]", self.build:getName(), "[TIME]", s}), 21, {255, 255, 255}, fixColor({89, 72, 18}))
    elseif bData["funcs"] == MINE_KIND then
        self.cw = colorWordsNode(getStr("mineInfo", {"[LEV]", str(self.build.buildLevel+1), "[NAME]", self.build:getName(), "[TIME]", s}), 21, fixColor({100, 100, 100}), fixColor({89, 72, 18}))
